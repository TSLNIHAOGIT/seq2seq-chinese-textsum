...
def model_with_buckets(encoder_inputs,
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      ...
      #因为之前encoder_inputs是取最长的bucket作为输入，这里我们要针对不同的bucket进行截取：
      #encoder_inputs[:bucket[0]]
      #这里的seq2seq函数就是embedding_attention_seq2seq()函数 
        ...
        if per_example_loss:
          #sequence_loss_by_example的作用：Weighted cross-entropy loss for a sequence of logits (per example).
          #这下理解了为什么要把target为'PAD'的weight设置为0,就是要让他们对loss不起作用（PAD只起填充作用）   
          losses.append(
              sequence_loss_by_example(
                  outputs[-1], #每次都去最后一个output
                  targets[:bucket[1]],
                  weights[:bucket[1]],
                  softmax_loss_function=softmax_loss_function))
        else:
